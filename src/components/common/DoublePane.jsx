...
export default function DoublePane({ children, title = '', leftPane = 3, defaultActivePane = 0, setActivePane = null }) {
  ...
  useEffect(() => {
    setPane(defaultActivePane)
  }, [defaultActivePane])
  // Trigger setActivePane callback to warn parent component of currently displayed item
  useEffect(() => {
    children.length && setActivePane && setActivePane(children[pane].props)
  }, [pane, children, setActivePane])

  return (
    <Card className={`double-pane ${title}`}>
      <Row>
        <Col lg={leftPane} className='tab-col'>
          {Array.isArray(children) ? children.map((x, i) => tabTitle(x.props.title, i)) : tabTitle(children.props.title, 0)}
